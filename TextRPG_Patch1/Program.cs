...
//------ tips -------
  ...
  const ConsoleKey C = ConsoleKey.C;
  const ConsoleKey V = ConsoleKey.V;
  const ConsoleKey Y = ConsoleKey.Y; // (YES)
  const ConsoleKey N = ConsoleKey.N; // (NO)
  const ConsoleKey ESC = ConsoleKey.Escape;
  ...
  // THE core loop of the game sort of
  static void HallLoop(ref bool exit) 
  {
    WriteLine("");
    WriteLine("Z. Look around");
    WriteLine("X. Go forward");
    ...
    switch (GetChoice()) // go forward, randomly generated room/hall
    {
      case Z:
        CheckSurroundings();
        break;
